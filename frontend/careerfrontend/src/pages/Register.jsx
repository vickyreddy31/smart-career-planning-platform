...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send register data excluding confirmPassword
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-indigo-950 p-6">
      <div className="w-full max-w-md bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-10 shadow-xl text-left">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-slate-950 dark:text-white mb-2">Create Account</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Get started with planning your career</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              id="username"
              name="username"
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 transition duration-200 text-slate-900 dark:text-white"
          ...
          <button
            type="submit"
            className="w-full py-3 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-xl shadow-lg shadow-purple-600/20 hover:shadow-purple-600/40 hover:-translate-y-0.5 transition duration-200 disabled:opacity-50 mt-4"
            disabled={loading}
          >
